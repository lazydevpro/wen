...
export const CHART_VERIFIER_ABI = [
    ...
    // Without these, every revert decodes as "unknown custom error" — which made an
    // already-recorded candle indistinguishable from one that genuinely cannot be proven.
    'error NotOwner()',
...
export const CHART_REGISTRY_ABI = [
    // merkleRoot, anchorSqrtPriceX96, totalCandles and visible are no longer sent — the registry
    // derives all four from the candle series after checking each candle against ChartVerifier.
    'function registerWindow((bytes32 windowId,address pool,uint256 bandHeight,uint16 visibleCount,uint8 timeSteps,uint8 priceBands,bool invert,string eraLabel,bytes32 riddleHash,uint32[] multipliers,uint64[] blockNumbers,uint160[] sqrtPrices) p) external',
...
export const GRID_GAME_ABI = [
    ...
    'function TOTAL_EXPOSURE_DIVISOR() external view returns (uint256)',
